...
import {
  FaMotorcycle, FaCheckCircle, FaMoneyBillWave,
  FaRoute, FaArrowRight, FaMapMarkerAlt,
} from "react-icons/fa";
import {
  MdElectricBolt, MdLocalShipping, MdVerified,
  MdOutlineSpeed, MdOutlinePendingActions,
} from "react-icons/md";
...
// ─────────────────────────────────────────────────
//  DESIGN TOKENS  — Rider = Amber
// ─────────────────────────────────────────────────
const ACC   = "#fbbf24";  // amber — rider brand
const SEC   = "#f59e0b";  // deeper amber
const CASH  = "#10b981";  // green — money settled
...
const StatusDot = ({ status }) => {
  const map = { assigned: ACC, "in-transit": INTER };
...
const StatCard = ({ icon: Icon, label, value, color, prefix = "", sub, delay = 0 }) => {
  ...
  return (
    ...
    >
      ...
      <div>
        <p className="text-[9px] font-black uppercase tracking-widest opacity-25 mb-1.5">{label}</p>
...
// ─────────────────────────────────────────────────
//  EARNINGS SUMMARY CARD  (the big hero card)
// ─────────────────────────────────────────────────
const EarningsHero = ({ stats }) => {
  const cashPct  = stats.totalEarned > 0
    ? Math.round((stats.cashedOut / stats.totalEarned) * 100) : 0;
      ...
      <div className="relative grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-base-content/6">
        ...
        <div className="p-7 flex flex-col gap-2">
          <div className="flex items-center gap-2 mb-1">
            <TbCoinFilled className="text-base" style={{ color: ACC }} />
            <span className="text-[9px] font-black uppercase tracking-[0.28em] opacity-30">
              Lifetime Earned
            </span>
          </div>
          <p className="text-4xl font-black tabular-nums leading-none" style={{ color: ACC }}>
        ...
        {/* Cashed / Pending split */}
        <div className="p-7 flex flex-col gap-5">
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.28em] opacity-30 mb-2">
              Settled
            </p>
            <p className="text-2xl font-black tabular-nums leading-none" style={{ color: CASH }}>
              ৳{stats.cashedOut.toLocaleString()}
            </p>
            <div className="flex items-center gap-1.5 mt-1.5">
              <MdVerified className="text-xs" style={{ color: CASH }} />
          ...
          <div>
            ...
            <p className="text-2xl font-black tabular-nums leading-none" style={{ color: PEND }}>
              ৳{stats.pendingCashout.toLocaleString()}
            </p>
            <div className="flex items-center gap-1.5 mt-1.5">
              <MdOutlinePendingActions className="text-xs" style={{ color: PEND }} />
              <span className="text-[9px] font-bold opacity-30">Awaiting cashout</span>
            </div>
          </div>
        </div>

        {/* Velocity (today / week / month) */}
        <div className="p-7 flex flex-col gap-4">
          ...
          {[
            ...
            <div key={label} className="flex items-center justify-between">
              ...
              <span className="font-black text-sm tabular-nums" style={{ color: ACC }}>
                ৳{earned}
              </span>
            </div>
          ))}
...
const ActiveTaskCard = ({ parcel, index }) => {
  ...
  return (
    <Motion.div
      ...
      className="flex items-stretch gap-0 rounded-2xl border border-base-content/5 bg-base-100 overflow-hidden"
    >
      {/* Left colour stripe */}
      ...
      <div className="flex-1 p-4 flex flex-col gap-3 min-w-0">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex flex-col items-end gap-1.5 shrink-0">
            ...
            {parcel.delivery_type && <DeliveryTypeBadge type={parcel.delivery_type} />}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-base-content/5">
          ...
          <span className="font-black text-sm" style={{ color: ACC }}>
            ৳{parcel.earning ?? Math.round(parcel.cost * (parcel.earning_rate ?? 0.3))}
          </span>
        </div>
      </div>
    </Motion.div>
...
// ─────────────────────────────────────────────────
//  DISTRICT BAR CHART (inter vs same)
// ─────────────────────────────────────────────────
  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      {[
        ...
        { label: "Bike",     value: `${profile.bikeBrand} · ${profile.bikeRegistration}` },
        { label: "Phone",    value: profile.phone     },
      ].map(({ label, value }) => (
        <div
          key={label}
          className="flex items-center justify-between px-5 py-3 border-b border-base-content/4.5 last:border-0"
        >
      ...
      {/* Quick KPIs inside profile */}
      <div className="grid grid-cols-2 gap-0 border-t border-base-content/5.5">
...
const RiderDashboard = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    staleTime:       60_000,
    refetchInterval: 30_000,   // riders need fresher data — 30s
  });
  ...
  const firstName = profile.name?.split(" ")[0] ?? user?.displayName?.split(" ")[0] ?? "Rider";
    ...
    <div className="space-y-8 font-urbanist pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5">
          <div          >
            ...
            <p className="text-sm font-medium opacity-35 mt-3 max-w-sm">
              {stats.activeTotal > 0
                ? `${stats.activeTotal} active task${stats.activeTotal > 1 ? "s" : ""} · ৳${stats.pendingCashout} pending cashout`
          ...
          >
            ...
            {stats.activeTotal > 0 && (
              <div
                className="flex items-center gap-2 px-4 py-2.5 rounded-2xl border font-black text-xs uppercase tracking-wider"
                style={{
                  borderColor: `${ACC}35`,
                  background:  `${ACC}10`,
                  color: ACC,
            ...
            <Link
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-2xl font-black text-xs uppercase tracking-wider transition-all hover:scale-[1.03] hover:shadow-xl active:scale-95"
              style={{ background: ACC, color: "#020617", boxShadow: `0 6px 24px ${ACC}35` }}
            >
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <StatCard
            ...
            delay={0.06}
            prefix="৳"
            sub={`৳${stats.cashedOut} cashed out`}
          />
          <StatCard
            icon={MdOutlineSpeed}
            label="This Week"
              ...
              <>
                ...
                {stats.activeTotal > activeParcels.length && (
                  <Link
                    ...
                    className="flex items-center justify-center gap-2 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-wider border border-base-content/8 hover:border-base-content/20 transition-all opacity-50 hover:opacity-80"
                  >
                    View All {stats.activeTotal} Tasks <FaArrowRight className="text-[9px]" />
                  </Link>
                )}
              </>
            ) : (
              <div
                className="flex flex-col items-center justify-center py-14 rounded-2xl border border-base-content/5 bg-base-100 gap-4"
              >
                <div
      ...
      <section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          {/* Right col: chart + cashout tip */}
          <div className="lg:col-span-5 flex flex-col gap-4">

            {/* Delivery type breakdown chart */}
            <Card accent={INTER} delay={0.08}>
              <CardHead icon={FaMapMarkerAlt} title="Route Analysis" color={INTER} />
            ...
            {/* Cashout nudge if pending */}
            {stats.pendingCashout > 0 && (
              <Motion.div
                initial={{ opacity: 0, y: 12 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className="relative rounded-2xl border border-base-content/5 bg-base-100 overflow-hidden p-5"
              >
                <div
                ...
                <p className="text-2xl font-black tabular-nums" style={{ color: ACC }}>
                  ৳{stats.pendingCashout}
                </p>
                <p className="text-[10px] opacity-30 font-medium mt-1 mb-4">
                  Pending from {recentDeliveries.filter(d => d.cashout_status !== "cashed_out").length} deliveries. Head to your earnings to withdraw.
                </p>
                <Link
      ...
      <div className="flex items-center justify-between pt-2 border-t border-base-content/5">
        ...
        <p
          ...
          style={{ color: ACC }}
        >
          <MdElectricBolt /> Live · 30s refresh
        </p>
      </div>
    </div>
  );
